  ...
  useEffect(() => {
    const fetchArtigo = async () => {
      try {
        ...
        if (data) {
          // Parsing de segurança para garantir que JSONBs vêm como arrays
          const parsedArtigo = {
            ...data,
          ...
          // Se tiver cores, seleciona a primeira por defeito
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 xl:gap-16">
            ...
            >
              ...
              <AnimatePresence mode="wait">
                <motion.img 
                  ...
                  // Lógica: se não tiver imagem na cor (ou não houver cor), usa a capa principal
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="bg-white p-8 rounded-3xl shadow-lg border border-institucional-blue/10 mt-auto">
              ...
              <form className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input type="text" name="empresa" onChange={handleFormChange} required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-institucional-blue/20 outline-none transition-all text-sm" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-2">{language === 'PT' ? 'Nome *' : 'Name *'}</label>
                    <input type="text" name="nome" onChange={handleFormChange} required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-institucional-blue/20 outline-none transition-all text-sm" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input type="email" name="email" onChange={handleFormChange} required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-institucional-blue/20 outline-none transition-all text-sm" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-2">{language === 'PT' ? 'Quantidade (SqFt) *' : 'Amount (SqFt) *'}</label>
                    <input type="number" min="1" name="sqft" onChange={handleFormChange} required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-institucional-blue/20 outline-none transition-all text-sm" />
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-2">{language === 'PT' ? 'Notas Adicionais' : 'Additional Notes'}</label>
                  <textarea rows={3} name="obs" onChange={handleFormChange} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-institucional-blue/20 outline-none transition-all text-sm resize-none"></textarea>
                </div>

                <button type="submit" onClick={(e) => { e.preventDefault(); alert("Integração de envio pendente.") }} className="w-full bg-institucional-blue text-white py-4 rounded-xl font-bold text-lg hover:bg-blue-900 transition-all shadow-md hover:-translate-y-0.5 mt-4">
